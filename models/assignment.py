...
def train_vae(model, train_loader, args, is_cvae=False):
    ...
    """
    # train_loader has shape 128
    # Namespace(is_cvae=False, load_weights=False, batch_size=128, num_epochs=10, latent_size=15, input_size=784, learning_rate=0.001)
    ...
    for images, label in tqdm(train_loader):
        with tf.GradientTape() as tape:
            if is_cvae:
                oh = one_hot(label, 10)
                x_hat, mu, logvar = model(images, oh)
            else:
